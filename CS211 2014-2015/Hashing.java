...
public class Hashing {
    static int size = 418279; 						//this is the size of the hash table - a prime number is best
    ...
    static int count=0;
    static double loadFactor = (double)array.length/(double)size ;

    public static void main (String[] args) throws IOException {
      File testFile = new File("C:\\Users\\Cieran\\Desktop\\dictionary.txt");     	//this is where the file to be sorted is loaded from
...
//this method takes in a word to look for and the strategy by which it has been placed in the hash table
         int probes = 1;
         int index = getHashKey(word)%size;
...
//the whole thing is added together and then moduloed to create a hash table index
    	...
    	{
    		value = value + (int)word.charAt(i)*modPow(512, i, size)%size;
    	}
...
//for example, you could just add up all of the letters in the word
      ...
      public static void fillLinearProbing(){
         ...
         for(int i=0; i<array.length;i++){ 		//go through all words
            int collisions=0;
            int index = getHashKey(array[i])%size;	//generate a hash key
            ...
            if(i%150==0){
                System.out.println(count + ") " +array[i] + " was placed in slot "+index+" of the hash table after "+collisions+" collisions");
                count++;
            }
      ...
      public static void fillQuadraticProbing(){
          ...
          {
            int collisions=0;
            int index = getHashKey(array[i])%size;
            int queries=1;
